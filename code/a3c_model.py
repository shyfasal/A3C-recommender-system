...
# Import XLNet dependencies
from transformers import XLNetTokenizer, XLNetModel
import torch

# Fungsi untuk inisialisasi model XLNet
def initialize_xlnet():
    # Menggunakan model pre-trained XLNet dan tokenizer
    tokenizer = XLNetTokenizer.from_pretrained('xlnet-base-cased')
    model = XLNetModel.from_pretrained('xlnet-base-cased')
    return tokenizer, model

# Fungsi untuk mendapatkan embedding dari MovieID menggunakan XLNet
def get_xlnet_embeddings(movie_ids, tokenizer, model):


    # Ubah MovieID menjadi string untuk tokenisasi
    movie_ids_as_text = [str(movie_id) for movie_id in movie_ids]

    # Tokenisasi MovieID
    inputs = tokenizer(movie_ids_as_text, return_tensors='pt', padding=True, truncation=True, max_length=128)

    # Feed tokenized input ke XLNet model
    outputs = model(**inputs)

    # Mengambil representasi embedding (hidden state)
...
class InteractiveModel(object):
    ...
    def __init__(self, sess, rnn_size, layer_size,
        ...
        self.is_reinforce = tf.placeholder(tf.int32, shape=[], name='is_reinforce')  # Tambahkan placeholder ini
        ...
        # Define rnn_output as the logits from the decoder before applying softmax
        ...
        self.value_target = tf.placeholder(tf.float32, shape=[None, None], name='value_target')
        with tf.variable_scope('critic'):
            critic_fc_layer = Dense(1, activation=None)  
            self.value_prediction = critic_fc_layer(self.rnn_output)

        
        self.critic_loss = tf.reduce_mean(tf.square(self.value_target - self.value_prediction))

       
        self.advantage = self.imediate_reward + self.account_ratio * self.value_prediction - self.value_prediction

        
        self.actor_loss = tf.reduce_mean(-tf.reduce_sum(
            tf.log(1e-8 + tf.reshape(self.rnn_output, [-1, decoder_vocab_size])) *
            tf.reshape(self.onehot_sample, [-1, decoder_vocab_size]) * self.advantage,
            axis=-1), name='actor_loss')

        
        self.total_loss = self.actor_loss + 0.5 * self.critic_loss  

        self.train_opt = tf.train.AdamOptimizer(self.lr, epsilon=1e-4)
        gradients = self.train_opt.compute_gradients(self.total_loss)
    ...
    def reinforcement_learn(self, user_interesting, user_masking, processes_length, init_state=None, s_token=0,
        ...
        process_length_scalar = int(processes_length[0])  

        feed_dict = {
            self.interesting: user_interesting,
            ...
            self.value_target: np.zeros((1, process_length_scalar)),  i
            self.is_reinforce: 1
        }
    ...
    def supervised_learn(self, user_interesting, user_masking, targets, processes_length, init_state=None, s_token=0, s_hit=1.0):
        ...
        process_length_scalar = processes_length[0]
        ...
        feed_dict = {self.interesting: user_interesting,
                    ...
                    self.sequence_length: processes_length,
                    self.target: targets,
                    self.value_target: np.zeros((1, process_length_scalar)),  
                    self.is_reinforce: 0}
